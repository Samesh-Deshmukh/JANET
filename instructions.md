...
|---|---|---|
| `espeak-ng` | text-to-speech — this is JANET's actual voice (`audio/tts.py`) | **yes** |
| `paplay`, `pactl` | plays that speech, and reads/sets volume (`utils/volume.py`) | **yes** |
...
`ffmpeg` and `bwrap` may say MISSING without breaking the core assistant —
without `bwrap` JANET just declines to run code, which is the safe answer.
Anything else missing will stop it working.
...
and will sit there for a minute. Every run after that is offline. Silero VAD
needs no download — it ships inside the pip package.
...
```bash
rm -rf JANET/ ~/.cache/whisper ~/.cache/huggingface
# and, if you installed it: ollama rm qwen3:14b
```

JANET writes nothing outside its own directory and those two caches — except
...
|---|---|---|
| `audio/tts.py` | `espeak-ng` → temp WAV → `paplay` | `say(phrase)` — one function, blocking |
...
**TTS** — `winsound` is in the standard library, so you don't even need a new
dependency:

```python
# audio/tts.py — Windows
import subprocess, tempfile, os, winsound
...
**TTS** — `say` is built in, needs no Homebrew package, and sounds considerably
better than espeak-ng:
...
def say(phrase):
    subprocess.run(["say", phrase], check=True)     # blocking; that's the contract
```

(If you'd rather keep espeak-ng's voice for consistency, `brew install espeak-ng`
and play the WAV with `afplay` instead of `paplay` — a one-word change.)
